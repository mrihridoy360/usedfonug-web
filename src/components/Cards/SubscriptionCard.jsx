    ...
    return (
        <div className={`card regular_card ${data.is_active ? 'active_card' : ""}`}>
            <div className="card-header">
                ...
                <div className="sub_details">
                    ...
                    <span className="price">
                        {data?.final_price !== 0 ? `$${data?.final_price}` : "Free"}
                        {
                            data?.price > data?.final_price && <span className='sale_price'>{CurrencySymbol}{data?.price}</span>
                        }
            ...
            <div className="card-footer">
                {!data.is_active ? (
                    <button onClick={(e) => handlePurchasePackage(e, data)}>
                        <span>
                            {t('choosePlan')}
                        </span>
