...
const SingleCategory = ({ slug }) => {
    ...
    const getSingleCatItem = async (page) => {
        ...
        try {
            if (search != "") {
                const res = await allItemApi.getItems({ sort_by: sortBy, min_price: MinMaxPrice?.min_price, max_price: MinMaxPrice?.max_price, country: Country, state: State, city: City, posted_since: DatePosted, page, search, category_slug: catId })
                data = res?.data
            } else {
                setIsLoading(true)
                const res = await allItemApi.getItems({ sort_by: sortBy, min_price: MinMaxPrice?.min_price, max_price: MinMaxPrice?.max_price, country: Country, state: State, city: City, posted_since: DatePosted, page, category_slug: catId })
                data = res?.data
            }
            if (data.error !== true) {
                if (page > 1) {
                                    ...
                                    <div className="row">
                                        {
                                            IsLoading ?
                                                ...
                                                (
                                                    SingleCatItem && SingleCatItem?.map((item, index) => (
                                                        view === "list" ? (
                                                            <div className="col-12" key={index}>
                                                                <Link sc href={userData?.id == item?.user_id ? `/my-listing/${item?.slug}` : `/product-details/${item.slug}`} prefetch={false}>
