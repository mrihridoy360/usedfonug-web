...
const FeaturedViewAll = ({ slug }) => {
    ...
    const [sortBy, setSortBy] = useState("default"); // Initial state set to empty string
    ...
    const handleLikedItems = (itemId, isLiked) => {
        setLikedItems(prevLikedItems => {
            if (isLiked) {
                // If the item is being liked, add it to the likedItems array
                return [...prevLikedItems, itemId];
            } else {
                // If the item is being disliked, remove it from the likedItems array
                return prevLikedItems.filter(id => id !== itemId);
            }
        });
    };

    const fetchItemsData = async (page = 1,) => {
        try {
            setIsLoading(true);
    ...
    const handleLoadMore = () => {
        if (currentPage < lastPage) {
            fetchItemsData(currentPage + 1, sortBy); // Pass current sorting option
        }
    };
    ...
    const handleChange = (event) => {
        ...
        fetchItemsData(1, selectedSortBy); // Fetch data with the new sorting option
    };
                                    ...
                                    <div className="row">
                                        {isLoading ? (
                                            ...
                                            itemsData && itemsData.map((ele, index) => (
                                                <div className="col-12" key={index}>
                                                    <Link href={ele?.user_id == userData?.id ? `/my-listing/${ele?.slug}` : `/product-details/${ele?.slug}`}>
